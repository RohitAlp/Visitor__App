...
class FlatBloc extends Bloc<FlatEvent, FlatState> {
  ...
  Future<void> _addFlat(AddFlatEvent event, Emitter<FlatState> emit) async {
  ...
  Future<void> _loadTowers(Emitter<FlatState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(availableTowers: towers, isLoadingTowers: false));
    } catch (e) {
      emit(
        state.copyWith(
  ...
  Future<void> _loadWingsForTower(String tower, Emitter<FlatState> emit) async {
    if (tower.isEmpty) {
      emit(state.copyWith(availableWings: [], isLoadingWings: false));
      return;
    }
    ...
    try {
      ...
      emit(state.copyWith(availableWings: wings, isLoadingWings: false));
      _validateForm(emit);
    } catch (e) {
      emit(
        state.copyWith(
  ...
  Future<void> _loadFloorsForWing(
    String tower,
    String wing,
    Emitter<FlatState> emit,
  ) async {
    if (tower.isEmpty || wing.isEmpty) {
      emit(state.copyWith(availableFloors: [], isLoadingFloors: false));
      return;
    }
    ...
    try {
      ...
      emit(state.copyWith(availableFloors: floors, isLoadingFloors: false));
      _validateForm(emit);
    } catch (e) {
      emit(
        state.copyWith(
